...
import { mockForecast, mockProducts } from '../../mocks/mockData';
...
export default function Forecasting() {
  ...
  const weeks = fc.forecast.map(f => f.week);
  const likely = fc.forecast.map(f => f.likely);
  const low = fc.forecast.map(f => f.low);
  const high = fc.forecast.map(f => f.high);
  ...
  const forecastTraces = [
    ...
    { x: weeks, y: baseline, type: 'scatter', mode: 'lines', name: 'Baseline', line: { color: '#64748B', width: 2, dash: 'dot' } },
    /* Anomaly point */
    { x: ['2026-W18'], y: [155], type: 'scatter', mode: 'markers', name: 'Anomaly', marker: { size: 12, color: '#EF4444', symbol: 'circle', line: { width: 2, color: 'white' } } },
  ];
  ...
  const accuracyTraces = [
    { x: ['W14', 'W15'], y: [78, 82], type: 'bar', name: 'Predicted', marker: { color: 'rgba(13,148,136,0.6)' } },
    { x: ['W14', 'W15'], y: [75, 80], type: 'bar', name: 'Actual', marker: { color: 'rgba(59,130,246,0.6)' } },
  ];
    ...
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 280px', gap: 'var(--space-4)' }}>
        ...
        <div>
          <div className="chart-container" style={{ marginBottom: 'var(--space-4)' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 'var(--space-3)' }}>
              ...
              <div style={{ display: 'flex', gap: 'var(--space-2)' }}>
                ...
                <span className="badge badge-muted">Confidence Range</span>
            ...
            <p style={{ fontSize: 'var(--font-size-xs)', color: 'var(--color-text-muted)', marginBottom: 'var(--space-3)' }}>
              Units predicted per week with 95% confidence interval
            </p>
            <Plot
              data={forecastTraces}
          ...
          <div style={{ marginBottom: 'var(--space-4)' }}>
            ...
            <div className="grid-3">
              {fc.driver_details.map((driver, i) => (
                ...
                {fc.forecast.map((week, i) => {
                  ...
                  return (
                    <tr key={week.week}>
                      ...
                      <td>{baseline[i]?.toLocaleString()}</td>
                      <td>
                        <div style={{
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-4)' }}>
          ...
          {/* Smart Insight */}
          <div className="intelligence-banner" style={{ padding: 'var(--space-4)' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-2)', marginBottom: 'var(--space-2)' }}>
              <span>✨</span>
              <span style={{ fontSize: 'var(--font-size-sm)', fontWeight: 600, color: 'var(--color-primary-light)' }}>Smart Insight</span>
            </div>
            <p style={{ fontSize: 'var(--font-size-xs)', color: 'var(--color-text-secondary)', lineHeight: 1.6 }}>
              Increasing safety stock by 15% for Week 18 is recommended to mitigate risk from forecasted Monsoon season surge.
            </p>
          </div>
        </div>
      </div>
